...
static uint32_t pqsamp_value_barrier(uint32_t value)
{
...
/* CS20 Algorithm 2; keep the protected u/v intermediates separate. */
                                   ...
                                   const pqsamp_word *right)
{
  ...
  for (i = 0; i < state->shares; i++)
  {
    ...
    for (j = i + 1U; j < state->shares; j++)
    {
      ...
      random[i][j] = pqsamp_value_barrier(random[i][j]);
  ...
  {
    unsigned j;
    uint32_t xi = pqsamp_value_barrier(x.share[i]);
    uint32_t zi = xi & pqsamp_value_barrier(y.share[i]);

    zi = pqsamp_value_barrier(zi);
    for (j = 0; j < state->shares; j++)
    {
      ...
      u = (~xi) & pqsamp_value_barrier(random[i][j]);
      v = pqsamp_value_barrier(y.share[j] ^ random[i][j]);
      term = xi & v;
      zi ^= pqsamp_value_barrier(u) ^ pqsamp_value_barrier(term);
      zi = pqsamp_value_barrier(zi);
    }
    result.share[i] = zi;
  }
...
/* Refresh every share pair before XOR recombination. */
                                  ...
                                  uint32_t *out)
{
  ...
  for (i = 0; i < state->shares; i++)
  {
    ...
    for (j = i + 1U; j < state->shares; j++)
    {
      ...
      refreshed[i] = pqsamp_value_barrier(refreshed[i] ^ random);
      refreshed[j] = pqsamp_value_barrier(refreshed[j] ^ random);
    }
  }
  for (i = 0; i < state->shares; i++)
  {
    result ^= pqsamp_value_barrier(refreshed[i]);
  }
  *out = pqsamp_value_barrier(result);
  return PQSAMP_OK;
}
